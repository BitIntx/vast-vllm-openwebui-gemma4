...
웹 검색, 이미지 검색, 이미지 URL inspection, 직접 영상 URL inspection용 OpenAPI tool server를 같이 넣어뒀습니다.
...
- 채팅 입력창의 도구 목록에서 `Web + Image + Video Tools` 또는 `search_web`, `search_images`, `inspect_image`, `inspect_video` 활성화
...
주의: `inspect_video`는 직접 접근 가능한 `mp4`, `webm`, `mov`, `m4v` 파일 URL만 처리합니다. YouTube/TikTok/X 같은 페이지 URL에서 영상을 다운로드하거나 프레임을 추출하는 기능은 아직 넣지 않았습니다.
