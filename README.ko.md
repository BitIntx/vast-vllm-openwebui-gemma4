...
`search_web`, `read_webpage`, `read_pdf`, `search_images`, `capture_webpage`, `resolve_media_url`, `ollama_status`는 vLLM/Ollama 없이도 동작합니다. `inspect_image`, `inspect_image_deep`, `inspect_video`는 설정한 모델 백엔드를 사용합니다. `ocr_image`는 로컬 EasyOCR을 사용합니다.
...
렌더링된 웹페이지 스크린샷을 쓰려면 시스템에 Chrome/Chromium이 없을 때 로컬 Chrome for Testing 런타임을 설치합니다:
...
- `inspect_image_deep`가 전체 이미지 1장과 확대 crop tile 여러 장을 Ollama vision에 보내 작은 글자, UI, 로고, 세부 물체를 더 자세히 봅니다.
- `ocr_image`가 EasyOCR로 텍스트, bounding box, confidence를 추출합니다. 기본 언어는 한국어와 영어입니다.
- `inspect_video`가 직접 접근 가능한 mp4/webm/mov/m4v 파일을 다운로드하고 ffmpeg로 프레임을 뽑아 Ollama vision에 보냅니다.
- inspection에서는 thinking이 기본으로 켜집니다. 모델이 전체 토큰 예산을 thinking에 써서 최종 답변이 비면, 기본적으로 thinking 없이 한 번 자동 재시도합니다.
...
`capture_webpage`가 반환하는 `http://127.0.0.1:9000/...png` 스크린샷 URL을 `inspect_image_deep`에 다시 넘기려면 로컬 미디어 서버도 실행합니다:
...
```text
read_pdf 도구로 PDF URL의 텍스트와 표를 추출해줘.
```

```text
capture_webpage 도구로 자바스크립트가 많은 페이지를 렌더링하고 screenshot.url을 inspect_image_deep에 넘겨서 화면을 분석해줘.
```

```text
ollama_status 도구로 현재 로딩된 모델, 메모리, 스왑, 서비스 상태, Ollama 상태를 보여줘.
```
...
```text
inspect_image_deep 도구로 이미지 URL을 보고 작은 글자, 로고, 보이는 텍스트까지 자세히 확인해줘.
```

```text
ocr_image 도구로 스크린샷 URL의 텍스트를 confidence와 함께 추출해줘.
```
